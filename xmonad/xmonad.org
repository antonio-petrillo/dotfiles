...
Config { font = "xft:Fira Code Retina:pixelsize=12:antialias=true:hinting=true"
       ...
       , sepChar = "%"
       , alignSep = "}{"
       , template = " <icon=haskell_20.xpm/> <fc=#534e54>|</fc> %UnsafeStdinReader% }{ %uname% | %cpu% | %memory% | %eno1% | %date% "
       }
#+end_src
* TODO [ ]
+ [ ] Add more documetation to the configuration
+ [ ] Obviously extend this configuration
  + [ ] Add a tabbed layout
  + [ ] Add sublayout configuration
+ [ ] Better support for xmobar
  + [X] Launch xmobar
  + [ ] Add mouse action
  + [ ] Add Theme
  + [ ] Don't let window cover the bar
